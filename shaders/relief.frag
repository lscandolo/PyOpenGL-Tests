...
void main(void)
{
  ...
  if (height_map.set){
    ...
    const int linear_iterations = 10;
    const int binary_iterations = 3;

    const float maxheight = 1.0;

    /* Texture vector corresponding plus height info*/
    vec3 tex_coords = vec3(tex_coords(height_map,ex_TexCoord),maxheight);

    /* Normalized tangent space view vector */
    vec3 tbnViewN = normalize(tbnView);

    /* Parallax step direction to traverse the heightmap */
    /* tbnViewN / tbnView.z gives a vector where z comp. is 1, so the xy 
       component is the parallax displacement if the heigh of the heightmap at 
       the required point is lowest*/
    /* Then we divide it by the iterations we are going to do*/
    /* The height factor is then multiplied to the xy component 
      It scales the parallax displacement (effectively doing the 
     same as if the height of the heightmap where multiplied)*/
    vec3 pllxStep = (tbnViewN / -tbnViewN.z) / float(linear_iterations);

    pllxStep.xy *= height_factor;

    /* Depth bias to fix antialiasing in steep eye dirs */
    float db = 1.0 + tbnViewN.z;
    db*=db;
    db=1.0-db*db;
    pllxStep.xy *= db;

    float h = texture2D(height_map.tex,tex_coords.xy).x * maxheight;

    for (int i = 0; i < linear_iterations; i++){

      tex_coords += pllxStep;
      h = texture2D(height_map.tex,tex_coords.xy).x * maxheight;
  ...
  vec3 light_source = normalize((in_Modelview *  vec4(0,1,0,0.0)).xyz);
